...
<div class="container-fluid">
	...
	<!-- Page Heading -->
	<h1 class="h3 mb-2 text-gray-800"><i class="fa fa-plus-circle"></i> Tambah Data Penduduk</h1>
	...
	<div class="card shadow mb-4">
			...
			<i class="fa fa-chevron-circle-down" aria-hidden="true"></i>
				...
				<div class="form-group">
					...
					<select id="id_kk" name="id_kk" autocomplete="off" class="form-control combobox @error('id_kk') is-invalid @enderror" style="width: 100%">
            			...
            			@foreach ($kk as $item)
						<option value="{{ $item->id_kk }}" {{ old('id_kk') == $item->id_kk ? 'selected' : null }}>{{ $item->no_kk }} ( {{ $item->alamat_klg }} )
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					...
					<select id="id_kelahiran" name="id_kelahiran" autocomplete="off" class="form-control combobox @error('id_kelahiran') is-invalid @enderror" style="width: 100%">
						...
						<option value="{{ $item->id_kelahiran }}" {{ old('id_kelahiran') == $item->id_kelahiran ? 'selected' : null }}>{{ $item->no_kelahiran }} ( Ayah: {{ $item->ayah }},- Ibu: {{ $item->ibu }},- Anak Ke:,- {{ $item->anak_ke }})
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					...
					<select id="id_kematian" name="id_kematian" autocomplete="off" class="form-control combobox @error('id_kematian') is-invalid @enderror" style="width: 100%">
						...
						<option value="{{ $item->id_kematian }}" {{ old('id_kematian') == $item->id_kematian? 'selected' : null }}>{{ $item->no_kematian}} ( {{ $item->tgl_kematian->format('d-m-Y') }})
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					...
					<select id="id_datang" name="id_datang" autocomplete="off" class="form-control combobox @error('id_datang') is-invalid @enderror" style="width: 100%">
						...
						<option value="{{ $item->id_datang }}" {{ old('id_datang') == $item->id_datang? 'selected' : null }}>{{ $item->no_datang}} ( {{ $item->tgl_datang->format('d-m-Y') }})
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					...
					<select id="id_pindah" name="id_pindah" autocomplete="off" class="form-control combobox @error('id_pindah') is-invalid @enderror" style="width: 100%">
						...
						<option value="{{ $item->id_pindah }}" {{ old('id_pindah') == $item->id_pindah? 'selected' : null }}>{{ $item->no_pindah}} ( {{ $item->tgl_pindah->format('d-m-Y') }})
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
		...
		{{-- END OPSIONAL --}}
				...
				<div class="form-group">
					...
					<select name="goldar" id="goldar" autocomplete="off" required value="{{ old('goldar') }}" class="form-control @error('goldar') is-invalid @enderror">
							<option value="" disabled selected hidden>- Pilih Goldar -</option>
							<option value="-" {{ old('goldar') == "-"  ? 'selected' : null }}>-</option>
							<option value="A" {{ old('goldar') == "A"  ? 'selected' : null }}>A</option>
							<option value="B" {{ old('goldar') == "B"  ? 'selected' : null }}>B</option>
							<option value="AB" {{ old('goldar') == "AB"  ? 'selected' : null }}>AB</option>
							<option value="O" {{ old('goldar') == "O"  ? 'selected' : null }}>O</option>
					</select>
					{{-- Erorr Message --}}
					@error('goldar')
					<div class="invalid-feedback">{{ $message }}</div>
				...
				<div class="form-group">
					...
					<input type="text" name="pekerjaan" id="pekerjaan" autocomplete="off" required value="{{ old('pekerjaan') }}"
				...
				<div class="form-group">
					...
					<select name="status_perkawinan" id="status_perkawinan" autocomplete="off" required value="{{ old('status_perkawinan') }}" class="form-control @error('status_perkawinan') is-invalid @enderror">
							<option value="" disabled selected hidden>- Pilih Status Perkawinan -</option>
							<option value="Kawin" {{ old('status_perkawinan') == "Kawin"  ? 'selected' : null }}>Kawin</option>
							<option value="Belum Kawin" {{ old('status_perkawinan') == "Belum Kawin"  ? 'selected' : null }}>Belum Kawin</option>
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					<label>Rt</label>
					<input type="text" name="rt" id="rt" autocomplete="off" required value="{{ old('rt') }}"
				...
				<div class="form-group">
					<label>Rw</label>
					<input type="text" name="rw" id="rw" autocomplete="off" required value="{{ old('rw') }}"
				...
				<div class="form-group">
					...
					<input type="text" name="kelurahan" id="kelurahan" autocomplete="off" required value="{{ old('kelurahan') }}"
				...
				<div class="form-group">
					...
					<input type="text" name="kecamatan" id="kecamatan" autocomplete="off" required value="{{ old('kecamatan') }}"
				...
				<div class="form-group">
					...
					<input type="text" name="kewarganegaraan" id="kewarganegaraan" autocomplete="off" required value="{{ old('kewarganegaraan') }}"
				...
				<div class="form-group">
					...
					<select name="status_pada_keluarga" id="status_pada_keluarga" autocomplete="off" value="{{ old('status_pada_keluarga') }}" class="form-control @error('status_pada_keluarga') is-invalid @enderror">
							<option value="" disabled selected hidden>- Pilih Status Pada Keluarga -</option>
							<option value="Ayah" {{ old('status_pada_keluarga') == "Ayah"  ? 'selected' : null }}>Ayah</option>
							<option value="Ibu" {{ old('status_pada_keluarga') == "Ibu"  ? 'selected' : null }}>Ibu</option>
							<option value="Anak" {{ old('status_pada_keluarga') == "Anak"  ? 'selected' : null }}>Anak</option>
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					...
					<select name="status_warga" id="status_warga" autocomplete="off" value="{{ old('status_warga') }}" class="form-control @error('status_warga') is-invalid @enderror">
							<option value="" disabled selected hidden>- Pilih Status Warga -</option>
							<option value="Tetap"  {{ old('status_warga') == "Tetap" ? 'selected' : null }}>Tetap</option>
							<option value="Kontrak"  {{ old('status_warga') == "Kontrak" ? 'selected' : null }}>Kontrak</option>
					</select>
					{{-- Erorr Message --}}
					@error('status_warga')
					<div class="invalid-feedback">{{ $message }}</div>
				...
				<div class="form-group">
					...
					<input type="text" name="keterangan" id="keterangan" autocomplete="off" required value="{{ old('keterangan') }}"
