...
<div class="container-fluid">
	...
	<!-- Page Heading -->
	<h1 class="h3 mb-2 text-gray-800"><i class="fa fa-plus-circle"></i> Tambah Data Kelahiran</h1>
		...
		<div class="card-body">
			...
			<form action="  {{ url('kelahiran') }} " method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<input type="text" name="no_kelahiran" autocomplete="off" required maxlength="30" value="{{ old('no_kelahiran', $nomer) }}"
				...
				<div class="form-group">
					...
					<select id="ayah" name="ayah" autocomplete="off" class="form-control combobox @error('ayah') is-invalid @enderror">
						...
						<option value="{{ $item->nama }}" {{ old('ayah') == $item->nama ? 'selected' : null }}>{{ $item->nama }} ( {{ $item->nik }} )
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					...
					@error('ibu') is-invalid @enderror">
						<option value="" selected>- Nama Ibu -</option>
            			@foreach ($penduduk as $item)
						<option value="{{ $item->nama }}" {{ old('ibu') == $item->nama ? 'selected' : null }}>{{ $item->nama }} ( {{ $item->nik }} )
						</option>
			            @endforeach 
					</select>
					{{-- Erorr Message --}}
				...
				<div class="form-group">
					<label>Anak Ke</label>
					<input type="text" name="anak_ke" autocomplete="off" maxlength="4" required value="{{ old('anak_ke') }}"
				...
				<div class="form-group">
					...
					<input type="text" name="penolong" autocomplete="off" maxlength="200" required value="{{ old('penolong') }}"
