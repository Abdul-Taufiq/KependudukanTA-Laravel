...
<div class="container-fluid">
	...
	<!-- Page Heading -->
	<h1 class="h3 mb-2 text-gray-800"><i class="fa fa-plus-circle"></i> Tambah Data User</h1>
		...
		<div class="card-body">
			...
			<form action="  {{ url('user') }} " method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
					<label>Nama</label>
					<input type="text" name="name" id="name" autocomplete="off" required maxlength="200" value="{{ old('name') }}"
				...
				<div class="form-group">
					<label>Email</label>
					<input type="email" name="email" id="email" autocomplete="off" required maxlength="200" value="{{ old('email') }}"
				...
				<div class="form-group">
					...
					<select name="level" id="level" autocomplete="off" required value="{{ old('level') }}" class="form-control @error('level') is-invalid @enderror">
							...
							<option value="KADES" {{ old('level') == "KADES"  ? 'selected' : null }}>KADES</option>
							<option value="SEKDES" {{ old('level') == "SEKDES"  ? 'selected' : null }}>SEKDES</option>
					</select>
					{{-- Erorr Message --}}
					@error('level')
					<div class="invalid-feedback">{{ $message }}</div>
